    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="space-y-4 rounded-xl border border-[#252a33] bg-[#13161c] p-5">
        ...
        <p className="text-sm text-[#8b93a7]">
          По умолчанию сервер использует <strong className="text-[#c4c9d4]">public-first</strong>: публичная карточка и
          popup детализации; buyer-login и импорт cookies — опциональный fallback при капче, auth wall или для
          card.wb.ru. Сессия покупателя не является обязательной для работы мониторинга.
        </p>
        ...
        {loginPayload?.autoLoginWindowSpawned ? (
          <p className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-100">
            Запущено окно браузера для входа (процесс отделён от вкладки с настройками). Проверьте Dock / Cmd+Tab. После
            входа в WB нажмите «Подтвердить вход».
          </p>
        ) : null}
        {loginPayload ? (
